...
**Where it stands** (v0.3.14, 2026-09-25): 56 MCP tools, a cluster of three Linux nodes and a
...
- [x] **5.5. Agent Fleet Stress Verification** — `divergent-bench --persistent --churn N` drops N% of sessions without a goodbye (simulated agent SIGKILL), reconnects and verifies the gateway retires every session: 24 workers, 960 queries, 193 dropped sessions, 0 errors, gateway healthy with 0 sessions afterwards (2026-09-19). The 50+ worker run followed on 2026-09-26 against a 1,251-file Rust repository with the same harness (`prod-code divergent-bench` takes the place of the planned `bench/masstest.py`): 56 persistent workers across four diverged worktrees (a signature change, a manifest change and an untracked file, all uncommitted), 1,120 queries, 20% churn (224 sessions dropped without a goodbye), 0 errors, no cross-worktree bleed, gateway healthy with 0 sessions afterwards. Cold, while the four worktrees' engines loaded: 93.6 s, p95 144 ms, p99 27.9 s. Warm: 4.18 s, 267.9 QPS, p50 61 ms, p99 215 ms, max 276 ms.
...
- [~] **7.1. Full-Spectrum AST Refactoring Engine (IDE-Grade Parity Catalog)** — status 2026-09-24: the catalog is complete for Rust (every item below is `[x]` or *not applicable to Rust*, except the two marked Rust-only). The other engines have rename (`code_rename`) and their own servers' code actions (`code_assists` / `code_assist`), but most of the tools built here (change_signature, move, encapsulate_field, …) read Rust. `extract_parameter` and `introduce_parameter_object` also work on TypeScript, JavaScript, Python, Go, C, C++ and Swift (#286, #287, #297); with them the tracked scope of #13 is done (closed 2026-09-25).
  ...
  - **7.1.2. The Extract & Introduce Family**:
    ...
    - [x] `refactor.extract_parameter(path, range, param_name)` — shipped 2026-09-22 for Rust, 2026-09-24 for TypeScript, JavaScript, Python and Go (#286: `name: T`, `name`, `name T`, the type from the literal or the server's hover; a rest parameter refuses), and the same day for C, C++ and Swift (#297: `T name` with the header prototype changed too, `name: T` or `_ name: T`); verified live against clangd and sourcekit-lsp, which found the check fixes #292–#294: `code_extract_parameter` / `prod-code extract-parameter <file> <line> <col> --to <line>:<col> --name …` add the parameter at the end of the list keeping its shape, replace the selection (or every identical occurrence with `replace_all`) in the body, and pass the original expression at every call site so no existing caller changes behaviour. An expression that names a local is refused with that as the reason.
