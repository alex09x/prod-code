...
pub fn list_tools() -> Vec<McpTool> {
    let mut tools = vec![
        ...
        McpTool {
            ...
            description: "Bundle several of a function's parameters into a new type, with the body and every call site, in Rust, TypeScript, Python, Go, C, C++ and Swift. `params` names the parameters to bundle (two or more, by the names the declaration gives them); they become fields of the new type, in declaration order and with the types the declaration gave: in Rust a `pub struct` written directly above the function (a single lifetime is introduced when any of those types borrows); in TypeScript an `interface` above the function or its class; in Python a `@dataclass` above the function or its class, with `from dataclasses import dataclass` added once (a plain class with an `__init__` when a parameter has no annotation and the analyzer cannot type it); in Go a `struct` whose fields keep the parameters' names; in C and C++ a `struct` declared the way the parameters were (a C++ default becomes the member's initialiser), written into the header above the prototype (above the class, for a method) when there is one, and every declaration of the function — the header's prototype and the definition — rewritten together; in Swift a `struct` of `let` properties (`public` when the function is), above the function or its type. The declaration takes one parameter in place of them, every use of them in the body is rewritten to reach through it (at the positions the analyzer reports, not by text search), and every call site passes one literal of the new type where the first bundled argument was (`Opts { a: x, b: y }`, TypeScript `{ a: x, b: y }`, Python `Opts(a=x, b=y)` with keyword arguments matched by name, Go `Opts{a: x, b: y}`, C `(struct Opts){.a = x, .b = y}`, C++ `{.a = x, .b = y}` when the project's build declares C++20 or later and `{x, y}` otherwise, Swift `Opts(a: x, b: y)` with arguments bound by their labels and passed under the new parameter's), so an argument that is a method chain or a closure survives and the unbundled arguments stay where they were. A caller in another Rust module gets the `use`, a Python caller gets the name added to its import from the declaring module. References that are not a call with the declaration's arity are named, not rewritten. The whole change is type-checked in one overlay before anything is written, and `apply` is what writes it. Re-run your formatter afterwards."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    ...
                    "name": { "type": "string", "description": "The new type's name, UpperCamelCase (`Opts`, `SyncRequest`); a Go type may be lower case, which leaves it unexported, and so may a C or C++ struct" },
                    "binding": { "type": "string", "description": "What the new parameter is called in the body (default: the type's name in snake_case in Rust, Python, C and C++, lowerCamelCase in TypeScript, Go and Swift)" },
